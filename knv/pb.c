...
static inline int checkreturn pb_get(pb_field_t *f, uint8_t *buf, size_t count)
{
    if (f->left < count)
...
static inline int checkreturn pb_get_byte(pb_field_t *f, uint8_t *buf)
{
    if (f->left < 1)
        PB_RETURN_ERROR(f, -1, "end-of-buffer");
...
static inline int checkreturn pb_get_dword(pb_field_t *f, uint32_t *buf)
{
    if (f->left < 4)
        PB_RETURN_ERROR(f, -1, "end-of-buffer");
...
static inline int checkreturn pb_get_ddword(pb_field_t *f, uint64_t *buf)
{
    if (f->left < 8)
        PB_RETURN_ERROR(f, -1, "end-of-buffer");
...
static inline int checkreturn pb_decode_varint(pb_field_t *f)
{
    uint8_t byte;
    uint8_t bitpos = 0;
    uint64_t result = 0;
...
static inline int checkreturn pb_decode_string(pb_field_t *f)
{
    uint32_t sz;
...
static inline int checkreturn pb_skip_varint(pb_field_t *f)
{
    uint8_t byte;
    do
    {
...
static inline int checkreturn pb_skip_string(pb_field_t *f)
{
    if (pb_decode_varint(f))
        return -1;
...
static inline int checkreturn pb_decode_tag(pb_field_t *f)
{
    if (pb_decode_varint(f))
    {
        if(f->left == 0)
...
int checkreturn pb_skip_field(pb_field_t *f, pb_wire_type_t wire_type)
{
    switch (wire_type)
    {
...
static inline int checkreturn pb_add(pb_buff_t *b, const void *buf, size_t count)
{
    if(b->left < count)
...
/*
static inline int checkreturn pb_add_byte(pb_buff_t *b, const uint8_t *buf)
{
    if(b->left < 1)
        PB_RETURN_ERROR(b, -1, "buf overflow");
...
*/
static inline int checkreturn pb_add_dword(pb_buff_t *b, uint32_t val)
{
    if(b->left < 4)
        PB_RETURN_ERROR(b, -1, "buf overflow");
...
static inline int checkreturn pb_add_ddword(pb_buff_t *b, uint64_t val)
{
    if(b->left < 8)
        PB_RETURN_ERROR(b, -1, "buf overflow");
...
static inline int checkreturn pb_add_vint(pb_buff_t *b, uint64_t value)
{
    uint8_t buffer[10];
    size_t i = 0;
...
static inline int checkreturn pb_add_tag(pb_buff_t *b, pb_wire_type_t wiretype, uint32_t field_number)
{
