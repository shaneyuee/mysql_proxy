/******************************************************************************
 ...
 HISTORY:
	...
	2014-03-15      Shaneyu        Multi-threading support
 ******************************************************************************/
...
#define HASH_MAX_KEY_LENGTH    10240
#define HASH_MAX_DATA_LENGTH   (100*1024*1024)
...
typedef struct {
	...
	uint16_t wMaxKeyLen;    // optional: max key length, use as small value as possible to save index space (default to 8)
...
public:
	MultiHashTable():header(NULL),ht(NULL),lt(),pShmAdpt(NULL),hashfunc(DefaultHashFunc)
	{
	...
	//     wMaxKeyLen     -- max key length, use as small value as possible to save index space
	...
	int InitFromBuffer(void *pBuffer, ull64_t ddwBufferSize, // required arguments 
			uint16_t wMaxKeyLen = DEFAULT_MAX_KEY_LENGTH,
	...
	int GetData(const void *pKey, int iKeyLen, char *sDataBuf, int &iDataLen, bool bUpdateAccessTime = true, time_t *pdwLastAccessTime = NULL);
	...
	//   0 -- 成功
	//  <0 -- 失败，请用 GetErrorMsg() 获取失败的原因
	int SetData(const void *pKey, int iKeyLen, const char *sDataBuf, int iDataLen, bool bRemoveOldIfNoSpace = false);
	...
	int PeekData(const void *pKey, int iKeyLen, char *sDataBuf, int &iDataLen) { return GetData(pKey, iKeyLen, sDataBuf, iDataLen, false); }
	// set data with bRemoveOldIfNoSpace=true
	int ForceSetData(const void *pKey, int iKeyLen, char *sDataBuf, int iDataLen) { return SetData(pKey, iKeyLen, sDataBuf, iDataLen, true); }
	...
	float GetHtUsage(); // 0~100, percentage
	float GetLtUsage(); // 0~100, percentage
};
