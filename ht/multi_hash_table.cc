/******************************************************************************
  ...
  请参考：
      ...
      http://km.oa.com/articles/show/182225

 HISTORY:
           Date          Author         Comment
        2013-11-12      Shaneyu        Created based on UDC's HashTable
 ******************************************************************************/
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
int MultiHashTable::InitFromBuffer(void *pBuffer, ull64_t ddwBufferSize, uint16_t wMaxKeyLen, uint16_t wRowNum, uint32_t dwIndexRatio, ull64_t ddwBlockSize, CalcHashKeyFunc HashFunc, bool bUseAccessSeq)
{
	if(pBuffer==NULL || ddwBufferSize<1024*1024*2 ||
		wMaxKeyLen<1 || wMaxKeyLen>HASH_MAX_KEY_LENGTH ||
	...
	ull64_t ddwNodeSize = EvaluateHashNodeSize(wMaxKeyLen);
...
int MultiHashTable::CreateFromShm(const MhtInitParam &user_param)
{
	...
	{
		snprintf(errmsg, sizeof(errmsg), "Bad argument, ShmKey(%lu) and BufferSize(%lu) must be given", param.ddwShmKey, param.ddwBufferSize);
		return -1;
	}
	if(param.wMaxKeyLen==0)   param.wMaxKeyLen = DEFAULT_MAX_KEY_LENGTH;
	...
	if((iret=InitFromShm(param.ddwShmKey, param.HashFunc))==0) // shm exists, check against given parameters
	{
		...
		if((int)param.wMaxKeyLen!=GetMaxKeyLen())
		{
			snprintf(errmsg, sizeof(errmsg), "Max key length mismatched: shm %d, given %d", GetMaxKeyLen(), (int)param.wMaxKeyLen);
			return -3;
		}
	...
	return InitFromBuffer(pBuffer, param.ddwBufferSize, param.wMaxKeyLen, param.wRowNum, param.dwIndexRatio, param.ddwBlockSize, param.HashFunc, param.bUseAccessSeq);
}
...
int MultiHashTable::GetData(const void *pKey, int iKeyLen, char *sDataBuf, int &iDataLen, bool bUpdateAccessTime, time_t *pdwLastAccessTime)
{
	MhtNode *pDataNode;
...
int MultiHashTable::SetData(const void *pKey, int iKeyLen, const char *sDataBuf, int iDataLen, bool bRemoveOldIfNoSpace)
{
	...
	if(ht_usage > 50) Attr_API(358676, 1);
	if(ht_usage > 80) Attr_API(358677, 1);
	if(ht_usage > 90) Attr_API(358678, 1);
	if(ht_usage > 95) Attr_API(358679, 1);
	...
	{
		Attr_API(391679, 1);
		snprintf(errmsg, sizeof(errmsg), "Mutex lock failed: %s", errnotostr(errno));
		return -3;
	}
...
unlock_out:
	...
	{
		Attr_API(391680, 1);
		snprintf(errmsg, sizeof(errmsg), "Mutex unlock failed: %s", errnotostr(errno));
	}
	return ret;
}
...
int MultiHashTable::Get(MhtIterator it, MhtData &data)
{
	...
	{
		snprintf(errmsg, sizeof(errmsg), "LinkTable GetData failed: %s", lt.GetErrorMsg());
		if(ret==-21 || // position gone insane
			ret==-22) // not enough buffer for data, but that's impossible, sizeof(data.data) is sure to be big enough
		{
			Attr_API(2639258, 1);
			memset(pDataNode, 0, sizeof(*pDataNode));
			return -21;
...
// 0~100, percentage
float MultiHashTable::GetHtUsage()
{
	return ((float)header->ddwMaxUsedRow) * 100 / header->ddwRowNum;
}

// 0~100, percentage
float MultiHashTable::GetLtUsage()
{
	return lt.GetUsage();
}
